...
export default function Wedding({ apiEndpoint = "/api/weddings" }) {
  ...
  function validate() {
    ...
    if (!form.groom_fullname) err.groom_fullname = "Groom full name is required";
    if (!form.bride_fullname) err.bride_fullname = "Bride full name is required";
    ...
    if (!form.groom_baptismal_cert) err.groom_baptismal_cert = "Groom baptismal certificate is required";
    if (!form.bride_baptismal_cert) err.bride_baptismal_cert = "Bride baptismal certificate is required";
    if (!form.groom_confirmation_cert) err.groom_confirmation_cert = "Groom confirmation certificate is required";
    if (!form.bride_confirmation_cert) err.bride_confirmation_cert = "Bride confirmation certificate is required";
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-6xl mx-auto p-6 bg-white rounded-lg shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium mb-1">User ID (uid)</label>
          <input name="uid" value={form.uid} onChange={handleChange} className={`w-full input px-3 py-2 border rounded ${errors.uid ? 'border-red-500' : 'border-gray-300'}`} />
          {errors.uid && <p className="text-xs text-red-500 mt-1">{errors.uid}</p>}
        </div>
